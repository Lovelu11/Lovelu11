...
from telethon.tl.types import InputPeer, InputPeerChannel, InputPeerUser
from telethon.rpcerrorlist import PeerFlood, UserPrivacyRestricted
...
print ("3. Add only 200 members in group each time otherwise you will get flood )
...
try:
    ...
    os.system('clear')
    banner(1)
    print(re+"[!] run python setup.py first y\n")
    sys.exit(1)
...
for user in users:
    ...
    try:
        ...
        if mode == 1:
            ...
            sys.exit(" Mode Selected. ")
        client(InviteToChannelRequest(target_group_entity, [user_to_add]))
        print("Waiting for 60-180 Seconds...")
        time.sleep(random.randrange(0, 5))
    except :
        print("Getting Flood  from telegram. Script is stopping now. Please try again after some time.")
